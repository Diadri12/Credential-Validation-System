    ...
    def list_tuple_directory(list_pass,list_defer,list_fail):
        ...
        for x in range(Num_of_items):
            if list_pass[x]==120:
                ...
                text_file = open("input_progression_data.txt", "a")
                txtString="Progress -" + str(list_pass[x])+ "," +str(list_defer[x])+"," + str(list_fail[x])
            ...
            if (list_pass[x]==100) and (list_defer[x]==0 or list_defer[x]==20)and (list_fail[x]==0 or list_fail[x]==20):
                ...
                text_file = open("input_progression_data.txt", "a")
                txtString="Progress (Module trailer) -" + str(list_pass[x])+ "," +str(list_defer[x])+"," + str(list_fail[x])
                text_file.write(txtString)
                text_file.write("\n")
                text_file.close()
                continue
            if (list_pass[x]==0)or (list_fail[x]==20)or (list_fail[x]==40)or (list_fail[x]==60):
                ...
                text_file = open("input_progression_data.txt", "a")
                txtString="Module retriever -" + str(list_pass[x])+ "," +str(list_defer[x])+"," + str(list_fail[x])
                text_file.write(txtString)
                text_file.write("\n")
                text_file.close()
                continue
            if (list_fail[x]==80)or (list_fail[x]==100)or (list_fail[x]==120):
                ...
                text_file = open("input_progression_data.txt", "a")
                txtString="Exclude -" + str(list_pass[x])+ "," +str(list_defer[x])+"," + str(list_fail[x])
                text_file.write(txtString)
                text_file.write("\n")
                text_file.close()
    list_tuple_directory(list_pass,list_defer,list_fail)
    print()
    def text_file(text_file):
        print("-------------------------------------------------------------")
        print("Text file")
        text_file = open("input_progression_data.txt","r")
        print(text_file.read())
        text_file.close()
        os.remove("input_progression_data.txt")
    text_file(text_file)
